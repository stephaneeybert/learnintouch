...
$imagePath = $rssUtils->imagePath;
$imageUrl = $rssUtils->imageUrl;
$imageSize = $rssUtils->imageSize;
...
if ($formSubmitted == 1) {
  ...
  $rssId = LibEnv::getEnvHttpPOST("rssId");
  ...
  if (count($warnings) == 0) {
    ...
    if ($rss = $rssUtils->selectById($rssId)) {
      $rss->setImage($image);
      $rssUtils->update($rss);
      }
...
$rssId = LibEnv::getEnvHttpGET("rssId");
if (!$rssId) {
  $rssId = LibEnv::getEnvHttpPOST("rssId");
  }
...
$panelUtils->addHiddenField('rssId', $rssId);
