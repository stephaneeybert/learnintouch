...
class DocumentCategoryDao extends Dao {

  var $tableName;

  function DocumentCategoryDao($dataSource, $tableName) {
    Dao::Dao($dataSource);
...
HEREDOC;
  ...
  function countDuplicateListOrderRows() {
    $sqlStatement = "SELECT count(distinct dc1.id) as count FROM $this->tableName dc1, $this->tableName dc2 where dc1.id != dc2.id and dc1.list_order = dc2.list_order";
    return($this->querySelect($sqlStatement));
  }
