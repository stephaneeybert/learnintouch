...
if ($formSubmitted == 1) {
  ...
  if (count($warnings) == 0) {
    ...
    $profileUtils->setLogoFilename($filename);
...
$filename = $profileUtils->getLogoFilename();
...
if ($filename) {
  if (!LibImage::isGif($filename)) {
    $encodedFilename = urlencode($filePath . $filename);
    $url = $gUtilsUrl . "/printImage.php?filename=" . $encodedFilename . "&width=120&height=";
    $panelUtils->addLine($panelUtils->addCell($mlText[1], "nbr"), "<img src='$url' border='0' title='' href=''>");
  } else {
    $panelUtils->addLine($panelUtils->addCell($mlText[1], "nbr"), "<img src='$fileUrl/$filename' border='0' title='' href=''>");
  }
  $panelUtils->addLine();
  $panelUtils->addLine($panelUtils->addCell($mlText[4], "nbr"), "$fileUrl/$filename");
  $panelUtils->addLine();
