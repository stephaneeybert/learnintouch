...
if ($formSubmitted == 1) {
  ...
  if ($deleteImage == 1) {
    ...
    if ($str = $fileUploadUtils->checkFileName($userfile_name)) {
      ...
      array_push($warnings, $str);
    } else if ($str = $fileUploadUtils->checkFileSize($userfile_size, $elearningQuestionUtils->imageFileSize)) {
      array_push($warnings, $str);
    } else if ($str = $fileUploadUtils->uploadFile($userfile, $userfile_name, $elearningQuestionUtils->imageFilePath)) {
...
$str .= "\n<form name='edit' id='edit' action='$gElearningUrl/teacher/corner/exercise/question/image.php' method='post' enctype='multipart/form-data'>";
...
$str .= "\n<div class='system_field'>" . $fileUploadUtils->getFileSizeMessage($elearningQuestionUtils->imageFileSize). "</div>";
...
$str .= "\n<input type='hidden' name='max_file_size' value='" . $elearningExerciseUtils->imageFileSize . "' />";
$str .= "\n<input type='hidden' name='elearningQuestionId' value='$elearningQuestionId' />";

$str .= "\n</form>";

$str .= "\n<div class='system_cancel_button'><a href='$gElearningUrl/teacher/corner/exercise/compose.php' style='text-decoration:none; vertical-align:middle;'><img src='$gImagesUserUrl/" . IMAGE_COMMON_CANCEL . "' style='vertical-align:middle;' /> $websiteText[5]</a></div>";
