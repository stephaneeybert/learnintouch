...
if ($formSubmitted == 1) {
  ...
  if (count($warnings) == 0) {
    ...
    if ($elearningLesson = $elearningLessonParagraphUtils->selectById($elearningLessonParagraphId)) {
      $elearningLesson->setAudio($audio);
      $elearningLessonParagraphUtils->update($elearningLesson);
    }
...
$str .= "\n<form name='edit' id='edit' action='$gElearningUrl/teacher/corner/lesson/paragraph/audio.php' method='post' enctype='multipart/form-data'>";
...
$str .= "\n<div class='system_field'>" . $fileUploadUtils->getFileSizeMessage($elearningLessonParagraphUtils->audioFileSize) . "</div>";
...
$str .= "\n<input type='hidden' name='max_file_size' value='" . $elearningLessonParagraphUtils->audioFileSize . "' />";
