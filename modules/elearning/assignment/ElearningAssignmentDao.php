...
HEREDOC;
  ...
  function selectByClassIdAndResultWithinSessionId($elearningClassId, $elearningSessionId, $start = false, $rows = false) {
    $sqlStatement = "SELECT SQL_CALC_FOUND_ROWS ea.* FROM ($this->tableName ea, " . DB_TABLE_ELEARNING_SUBSCRIPTION . " esu, " . DB_TABLE_ELEARNING_SESSION . " es, " . DB_TABLE_ELEARNING_RESULT . " er, " . DB_TABLE_USER . " u) WHERE ea.elearning_subscription_id = esu.id AND u.id = esu.user_account_id AND esu.class_id = '$elearningClassId' AND (ea.elearning_result_id = er.id OR ea.elearning_result_id IS NULL) AND er.exercise_datetime IS NOT NULL AND ((es.opening_date IS NOT NULL AND es.closing_date IS NOT NULL AND DATE(er.exercise_datetime) >= DATE(es.opening_date) AND DATE(er.exercise_datetime) <= DATE(es.closing_date)) OR (es.opening_date IS NOT NULL AND es.closing_date IS NULL AND DATE(er.exercise_datetime) >= DATE(es.opening_date)) OR (es.opening_date IS NULL AND es.closing_date IS NOT NULL AND DATE(er.exercise_datetime) <= DATE(es.closing_date))) GROUP BY er.id ORDER BY er.exercise_datetime DESC, er.firstname, er.lastname";
    if ($rows) {
      if (!$start) {
        $start = 0;
      }
  ...
  function selectByResultWithinSessionId($elearningSessionId, $start = false, $rows = false) {
    $sqlStatement = "SELECT SQL_CALC_FOUND_ROWS ea.* FROM ($this->tableName ea, " . DB_TABLE_ELEARNING_SESSION . " es, " . DB_TABLE_ELEARNING_RESULT . " er, " . DB_TABLE_ELEARNING_SUBSCRIPTION . " esu, " . DB_TABLE_USER . " u) WHERE ea.elearning_subscription_id = esu.id AND u.id = esu.user_account_id AND (ea.elearning_result_id = er.id OR ea.elearning_result_id IS NULL) AND er.exercise_datetime IS NOT NULL AND ((es.opening_date IS NOT NULL AND es.closing_date IS NOT NULL AND DATE(er.exercise_datetime) >= DATE(es.opening_date) AND DATE(er.exercise_datetime) <= DATE(es.closing_date)) OR (es.opening_date IS NOT NULL AND es.closing_date IS NULL AND DATE(er.exercise_datetime) >= DATE(es.opening_date)) OR (es.opening_date IS NULL AND es.closing_date IS NOT NULL AND DATE(er.exercise_datetime) <= DATE(es.closing_date))) GROUP BY er.id ORDER BY er.exercise_datetime DESC, er.firstname, er.lastname";
    if ($rows) {
      if (!$start) {
        $start = 0;
      }
