...
$strCommand = ''
  ...
  . "<img border='0' src='$gCommonImagesUrl/$gImageLesson' title='$mlText[52]'></a>"
...
$strCommand = "<a href='$gElearningUrl/exercise/edit.php' $gJSNoStatus>"
...
$strCommand .= " <a href='$gElearningUrl/exercise/garbage.php' $gJSNoStatus>"
...
if ($searchPattern) {
  $elearningExercises = $elearningExerciseUtils->selectLikePattern($searchPattern, $listIndex, $listStep);
} else if ($elearningCourseId > 0) {
  ...
  $elearningExercises = $elearningExerciseUtils->selectByCategoryIdAndLevelIdAndSubjectIdAndReleaseDate($elearningCategoryId, $elearningLevelId, $elearningSubjectId, $sinceDate, $systemDate, $listIndex, $listStep);
} else if ($elearningCategoryId > 0 && $elearningLevelId > 0 && $elearningSubjectId > 0) {
  $elearningExercises = $elearningExerciseUtils->selectByCategoryIdAndLevelIdAndSubjectId($elearningCategoryId, $elearningLevelId, $elearningSubjectId, $listIndex, $listStep);
} else if ($elearningCategoryId > 0 && $elearningLevelId > 0 && $sinceDate) {
  $elearningExercises = $elearningExerciseUtils->selectByCategoryIdAndLevelIdAndReleaseDate($elearningCategoryId, $elearningLevelId, $sinceDate, $systemDate, $listIndex, $listStep);
} else if ($elearningCategoryId > 0 && $elearningSubjectId > 0 && $sinceDate) {
  $elearningExercises = $elearningExerciseUtils->selectByCategoryIdAndSubjectIdAndReleaseDate($elearningCategoryId, $elearningSubjectId, $sinceDate, $systemDate, $listIndex, $listStep);
} else if ($elearningLevelId > 0 && $elearningSubjectId > 0 && $sinceDate) {
  $elearningExercises = $elearningExerciseUtils->selectByLevelIdAndSubjectIdAndReleaseDate($elearningLevelId, $elearningSubjectId, $sinceDate, $systemDate, $listIndex, $listStep);
} else if ($elearningCategoryId > 0 && $elearningLevelId > 0) {
  $elearningExercises = $elearningExerciseUtils->selectByCategoryIdAndLevelId($elearningCategoryId, $elearningLevelId, $listIndex, $listStep);
} else if ($elearningCategoryId > 0 && $elearningSubjectId > 0) {
  $elearningExercises = $elearningExerciseUtils->selectByCategoryIdAndSubjectId($elearningCategoryId, $elearningSubjectId, $listIndex, $listStep);
} else if ($elearningLevelId > 0 && $elearningSubjectId > 0) {
  $elearningExercises = $elearningExerciseUtils->selectByLevelIdAndSubjectId($elearningLevelId, $elearningSubjectId, $listIndex, $listStep);
} else if ($elearningCategoryId > 0 && $sinceDate) {
  $elearningExercises = $elearningExerciseUtils->selectByCategoryIdAndReleaseDate($elearningCategoryId, $sinceDate, $systemDate, $listIndex, $listStep);
} else if ($elearningCategoryId > 0) {
  $elearningExercises = $elearningExerciseUtils->selectByCategoryId($elearningCategoryId, $listIndex, $listStep);
} else if ($elearningLevelId > 0 && $sinceDate) {
  $elearningExercises = $elearningExerciseUtils->selectByLevelIdAndReleaseDate($elearningLevelId, $sinceDate, $systemDate, $listIndex, $listStep);
} else if ($elearningLevelId > 0) {
  $elearningExercises = $elearningExerciseUtils->selectByLevelId($elearningLevelId, $listIndex, $listStep);
} else if ($elearningSubjectId > 0 && $sinceDate) {
  $elearningExercises = $elearningExerciseUtils->selectBySubjectIdAndReleaseDate($elearningSubjectId, $sinceDate, $systemDate, $listIndex, $listStep);
} else if ($elearningSubjectId > 0) {
  $elearningExercises = $elearningExerciseUtils->selectBySubjectId($elearningSubjectId, $listIndex, $listStep);
} else if ($publicAccessStatus == ELEARNING_PUBLIC && $sinceDate) {
  $elearningExercises = $elearningExerciseUtils->selectPublicAccessAndReleaseDate($sinceDate, $systemDate, $listIndex, $listStep);
} else if ($publicAccessStatus == ELEARNING_PUBLIC) {
  $elearningExercises = $elearningExerciseUtils->selectPublicAccess($listIndex, $listStep);
} else if ($publicAccessStatus == ELEARNING_PROTECTED && $sinceDate) {
  $elearningExercises = $elearningExerciseUtils->selectProtectedAndReleaseDate($sinceDate, $systemDate, $listIndex, $listStep);
} else if ($publicAccessStatus == ELEARNING_PROTECTED) {
  $elearningExercises = $elearningExerciseUtils->selectProtected($listIndex, $listStep);
} else if ($sinceDate) {
  $elearningExercises = $elearningExerciseUtils->selectByReleaseDate($sinceDate, $systemDate, $listIndex, $listStep);
} else {
  $defaultListEmpty = $preferenceUtils->getValue("ELEARNING_LIST_DEFAULT_EMPTY");
  if (!$defaultListEmpty) {
    $elearningExercises = $elearningExerciseUtils->selectNonGarbage($listIndex, $listStep);
  } else {
...
if (isset($elearningExercises)) {
  ...
  foreach ($elearningExercises as $elearningExercise) {
    ...
    $categoryId = $elearningExercise->getCategoryId();
    $subjectId = $elearningExercise->getSubjectId();
    $public = $elearningExercise->getPublicAccess();
    $locked = $elearningExercise->getLocked();
    $releaseDate = $elearningExercise->getReleaseDate();

    $strPageUrl = "$mlText[36] <br><br>$gElearningUrl/exercise/display_exercise.php?elearningExerciseId=$elearningExerciseId<br><br>$mlText[37]";
    ...
    if (!$elearningExerciseUtils->isLockedForLoggedInAdmin($elearningExerciseId)) {
      $strCommand .= " <a href='$gElearningUrl/exercise/edit.php?elearningExerciseId=$elearningExerciseId' $gJSNoStatus>"
        . "<img border='0' src='$gCommonImagesUrl/$gImageEdit' title='$mlText[42]'></a>"
        ...
        . " <a href='$gElearningUrl/course/addExercise.php?elearningExerciseId=$elearningExerciseId' $gJSNoStatus>"
        . "<img border='0' src='$gCommonImagesUrl/$gImageAddTo' title='$mlText[8]'></a>"
        . " <a href='$gElearningUrl/exercise/addToLesson.php?elearningExerciseId=$elearningExerciseId' $gJSNoStatus>"
        . "<img border='0' src='$gCommonImagesUrl/$gImageAddToLesson' title='$mlText[23]'></a>";
    }

    $adminLogin = $adminUtils->checkAdminLogin();
    if ($adminUtils->isSuperAdmin($adminLogin)) {
      if ($locked) {
        $strCommand .= " <a href='$gElearningUrl/exercise/lock.php?elearningExerciseId=$elearningExerciseId&locked=0' $gJSNoStatus>"
          . "<img border='0' src='$gCommonImagesUrl/$gImageUnlock' title='$mlText[25]'></a>";
      } else {
        $strCommand .= " <a href='$gElearningUrl/exercise/lock.php?elearningExerciseId=$elearningExerciseId&locked=1' $gJSNoStatus>"
          . "<img border='0' src='$gCommonImagesUrl/$gImageLock' title='$mlText[24]'></a>";
      }
    }

    $strCommand .= ' ' . $popupUtils->getDialogPopup("<img border='0' src='$gCommonImagesUrl/$gImagePreview' title='$mlText[26]'>", "$gElearningUrl/exercise/preview.php?elearningExerciseId=$elearningExerciseId", 600, 600)
    ...
    if (!$elearningExerciseUtils->isLockedForLoggedInAdmin($elearningExerciseId)) {
      $strCommand .= " <a href='$gElearningUrl/exercise/delete.php?elearningExerciseId=$elearningExerciseId' $gJSNoStatus>"
  ...
  foreach ($elearningCourseItems as $elearningCourseItem) {
    ...
    if ($elearningExerciseId) {
      if ($elearningExercise = $elearningExerciseUtils->selectById($elearningExerciseId)) {
        ...
        $strSwap = "<a href='$gElearningUrl/course/swapup.php?elearningExerciseId=$elearningExerciseId&elearningCourseId=$elearningCourseId&redirectToExercise=1' $gJSNoStatus><img border='0' src='$gCommonImagesUrl/$gImageUp' title='$mlText[11]'></a>"
          . " <a href='$gElearningUrl/course/swapdown.php?elearningExerciseId=$elearningExerciseId&elearningCourseId=$elearningCourseId&redirectToExercise=1' $gJSNoStatus><img border='0' src='$gCommonImagesUrl/$gImageDown' title='$mlText[10]'></a>";
      ...
      if ($elearningLesson = $elearningLessonUtils->selectById($elearningLessonId)) {
        ...
        $lessonExerciseLinks = $elearningLessonUtils->renderLessonExerciseComposeLinks($elearningLessonId, $mlText[21]);

        $strSwap = "<a href='$gElearningUrl/course/swapup.php?elearningLessonId=$elearningLessonId&elearningCourseId=$elearningCourseId&redirectToExercise=1' $gJSNoStatus><img border='0' src='$gCommonImagesUrl/$gImageUp' title='$mlText[11]'></a>"
          . " <a href='$gElearningUrl/course/swapdown.php?elearningLessonId=$elearningLessonId&elearningCourseId=$elearningCourseId&redirectToExercise=1' $gJSNoStatus><img border='0' src='$gCommonImagesUrl/$gImageDown' title='$mlText[10]'></a>";
        ...
        $strCommand .= " <a href='$gElearningUrl/lesson/edit.php?elearningLessonId=$elearningLessonId' $gJSNoStatus>"
          ...
          . "<img border='0' src='$gCommonImagesUrl/$gImageRemoveFrom' title='$mlText[15]'></a>"
          . ' ' . $popupUtils->getDialogPopup("<img border='0' src='$gCommonImagesUrl/$gImagePreview' title='$mlText[27]'>", "$gElearningUrl/lesson/preview.php?elearningLessonId=$elearningLessonId", 600, 600)
          . ' ' . $popupUtils->getDialogPopup("<img border='0' src='$gCommonImagesUrl/$gImagePrinter' title='$mlText[41]'>", "$gElearningUrl/lesson/print_lesson.php?elearningLessonId=$elearningLessonId", 600, 600)
        ...
        $strName = "<span class='drag_and_drop' elearningCourseItemId='$elearningCourseItemId'>" . $elearningLessonUtils->renderLessonComposeLink($elearningLessonId, $mlText[16]) . '</span>';
...
$strRememberScroll = LibJavaScript::rememberScroll("elearning_exercise_admin_vscroll");
