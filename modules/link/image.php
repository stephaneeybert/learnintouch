...
if ($formSubmitted == 1) {
  ...
  if (count($warnings) == 0) {
    ...
    if ($imageWidth) {
      if ($fileUploadUtils->isImageType($linkUtils->imageFilePath . $image) && !$fileUploadUtils->isGifImage($linkUtils->imageFilePath . $image)) {
        LibImage::resizeImageToWidth($linkUtils->imageFilePath . $image, $imageWidth);
      }
    }
  ...
  $imageWidth = $linkUtils->getImageWidth();
