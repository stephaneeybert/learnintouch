...
if ($formSubmitted) {
  ...
  // Set the page
  if ($isPhone) {
    $shopOrderUtils->setPhoneBankDetailsPage($languageCode, $url);
    } else {
    $shopOrderUtils->setComputerBankDetailsPage($languageCode, $url);
    }

  $str = LibHtml::urlRedirect("$gShopUrl/payment/transfert/bank.php");
  printContent($str);
  return;
  ...
  if ($isPhone) {
    $url = $shopOrderUtils->getPhoneBankDetailsPage($languageCode);
    } else {
    $url = $shopOrderUtils->getComputerBankDetailsPage($languageCode);
    }
  ...
  $panelUtils->setHeader($mlText[0], "$gShopUrl/payment/transfert/bank.php");
